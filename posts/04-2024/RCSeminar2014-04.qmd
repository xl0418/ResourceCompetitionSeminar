  ...
  for(ti in 1:tt){
    ...
    for(tii in 1:100){
      if(mode == "ess"){
        ...
        c11 <- v11* min(growth_rate(mu11, k11, temp_R1), growth_rate(mu12, k12, temp_R2))
        c12 <- v12* min(growth_rate(mu11, k11, temp_R1), growth_rate(mu12, k12, temp_R2))
        c21 <- v21*min(growth_rate(v21, k21, temp_R1), growth_rate(v22, k22, temp_R2))
        c22 <- v22*min(growth_rate(v21, k21, temp_R1), growth_rate(v22, k22, temp_R2))
...
ui <- fluidPage(
  ...
  fluidRow(
    column(4,
           div(style="height: 80px;", sliderInput('m', HTML("m"), 0.2, min = 0.1, max = 1))
    ),
    column(4,
  ...
  output$plot <- renderPlotly({
    if(input$models == "ess"){
      ...
      if((R1_1() - R2_1()) * (R1_2() - R2_2()) < 0){
        ...
        arrows <- reactive({data.frame(start_x = c(input$v11 * 100, input$v21 * 100),
                                       start_y = c(R_eq_y - (R_eq_x - input$v11 * 100) * input$v12 / input$v11,
                                                   R_eq_y - (R_eq_x - input$v21 * 100) * input$v22 / input$v21),
      ...
      A <- matrix(c(input$mu11, input$mu12, input$mu21, input$mu22), 2, 2)
      b <- c(input$m, input$m)
      sol <- solve(A, b)
      R_eq_x <- sol[1]
      R_eq_y <- sol[2]
